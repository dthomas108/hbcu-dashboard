...
import { UndergradEarningsTimeline } from "./charts/UndergradEarningsTimeline";
import { TopDegreesAwardedBar } from "./charts/TopDegreesAwardedBar";
...
export function Dashboard() {
  ...
  const effectiveId =
    selectedId || (schools[0]?.id ? String(schools[0].id) : "");
  ...
  const { data: detailData } = useSWR<{ results: ScorecardSchool[] }>(
    detailUrl,
    fetcher,
  );
    ...
    <DashboardShell>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between w-full">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <ToggleGroup
            type="single"
            value={type}
            onValueChange={(v) => v && setType(v as "2" | "4")}
          >
          ...
          <ToggleGroup
            type="single"
            value={hbcuOnly}
            onValueChange={(v) => v && setHbcuOnly(v as "0" | "1")}
          >
          ...
          <Select value={effectiveId} onValueChange={(v) => setSelectedId(v)}>
            <SelectTrigger className="w-[280px]">
              <SelectValue
                placeholder={
                  listLoading ? "Loading schools..." : "Select a school"
                }
              />
            </SelectTrigger>
            <SelectContent>
              {hbcuSchools.filter(Boolean).map((scorecard: ScorecardSchool) => (
                <SelectItem key={scorecard.id} value={String(scorecard.id)}>
                  {scorecard.school?.name ?? `School ${scorecard.id}`}
                  {scorecard.school?.state
                    ? ` (${scorecard.school.state})`
                    : ""}
                </SelectItem>
              ))}
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          ...
          subtitle={
            type === "2" ? "Typically not applicable to 2-year" : undefined
          }
        />
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <TopDegreesAwardedBar institutionName={school?.school?.name ?? ""} />
        <UndergradEarningsTimeline
          institutionId={school?.id ? String(school.id) : ""}
        />
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          ...
          <CardContent className="text-sm space-y-2 flex-1">
            ...
            <div>
              <span className="text-muted-foreground">Age 25+ share:</span>{" "}
              {formatPct(school?.latest?.student?.share_25_older)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-base">
              Institutional Information toggle here
            </CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground flex-1">
            Filter by “highest degree awarded” to 2-year vs 4-year, & show/hide
            admissions.
          </CardContent>
        </Card>

        <div className="disclaimer text-xs text-muted-foreground">
          *Data may be incomplete or inaccurate; contact us if you find any
          issues. Initial version only uses a subset of available Scorecard
          data, and is intended as a starting point for exploration. Future
          versions may add more indicators, charts, and interactivity. See the{" "}
          <a
            href="https://ed-data.github.io/scorecard/"
            target="_blank"
